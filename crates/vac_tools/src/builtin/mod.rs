...
pub async fn register_builtin_tools(registry: &Arc<ToolRegistry>) -> Result<(), ToolError> {
    ...
    registry
        .register(signal_tail::SignalTailTool::new())
        .await?;
    registry
        .register(signal_list::SignalListTool::new())
        .await?;
    registry
        .register(signal_distilled::SignalDistilledTool::new())
        .await?;
    registry
        .register(plan_mode::EnterPlanModeTool::new())
        .await?;
    registry
        .register(plan_mode::ExitPlanModeTool::new())
        .await?;
    registry
        .register(worktree::EnterWorktreeTool::new())
        .await?;
    registry.register(worktree::ExitWorktreeTool::new()).await?;
    registry
        .register(schedule_cron::ScheduleCronTool::new())
        .await?;
    ...
    registry
        .register(hook_registry::HookListTool::new())
        .await?;
    registry
        .register(hook_registry::HookDeleteTool::new())
        .await?;
    registry
        .register(hook_registry::HookCreateTool::new())
        .await?;
    ...
    let host = std::sync::Arc::new(crate::rust_analysis::PortablePtyHost::new().unwrap_or_else(
        |_| {
            tracing::warn!(
                "Failed to initialize PortablePtyHost, falling back to StubAnalysisHost"
            );
    ...
    registry
        .register(rust_analysis::RustSymbolLookup::new(host.clone()))
        .await?;
    registry
        .register(rust_analysis::RustDiagnostics::new(host))
        .await?;
