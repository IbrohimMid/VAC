...
impl VilTool for EnterWorktreeTool {
    ...
    async fn execute(
        ...
        let path = input.path.unwrap_or_else(|| {
            format!(".vac/worktrees/{}", sanitize_branch_dir(&input.branch))
        });
        ...
        let _ = std::fs::write(
            &lock_path,
            serde_json::to_vec_pretty(&lock_body).unwrap_or_default(),
        );
...
impl VilTool for ExitWorktreeTool {
    ...
    async fn execute(
        ...
        let lock_removed = std::fs::remove_file(&lock_path).is_ok();
