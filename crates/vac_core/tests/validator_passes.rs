...
use vil_ir::types::{IrFunction, IrModule, IrStruct, IrUse, TypeRef, Visibility, FnParam};
...
fn make_fn(name: &str, params: Vec<(&str, &str)>, ret: Option<&str>, is_async: bool) -> IrFunction {
    IrFunction {
        ...
        is_const: false,
        generics: vec![],
        params: params.into_iter().map(|(n, t)| FnParam {
            name: n.to_string(),
            ty: TypeRef { name: t.to_string(), generics: vec![], is_reference: false, is_mutable: false, lifetime: None, is_option: false, is_result: false },
            is_self: false,
            is_mutable: false,
            is_reference: false,
            lifetime: None,
        }).collect(),
        return_type: ret.map(|r| TypeRef { name: r.to_string(), generics: vec![], is_reference: false, is_mutable: false, lifetime: None, is_option: false, is_result: false }),
        where_clauses: vec![],
        body_summary: None,
        doc_comment: None,
        line_span: (0, 0),
...
fn clean_vil_handler_passes_all() {
    ...
    module.uses.push(IrUse { path: "tracing".to_string(), alias: None, visibility: Visibility::Private });
    let mut f = make_fn("create_order", vec![("ctx", "ServiceCtx"), ("slice", "ShmSlice")], Some("VilResponse"), true);
    f.vil_attrs = vec!["vil_handler".to_string()];
    module.functions.push(f);

    let report = run_all_passes(&module);
    assert!(report.score > 0.9, "clean VIL handler should score > 0.9, got {}", report.score);
    // Advisories (observability hints) are allowed — only hard violations fail
    let hard_issues: Vec<_> = report.issues.iter()
        .filter(|i| !i.starts_with("Advisory"))
        .collect();
    assert!(hard_issues.is_empty(), "clean handler should have no hard issues: {:?}", hard_issues);
}
...
fn json_extractor_fails_vil_way_pass() {
    ...
    assert!(report.issues.iter().any(|i| i.contains("Json")), "should flag Json<T>: {:?}", report.issues);
}

#[test]
fn extension_extractor_fails_vil_way_pass() {
    let mut module = make_module("ext_handler");
    let f = make_fn("ext_handler", vec![("state", "Extension")], Some("VilResponse"), true);
    module.functions.push(f);

    let report = run_all_passes(&module);
    assert!(report.issues.iter().any(|i| i.contains("Extension")), "should flag Extension<T>: {:?}", report.issues);
}
...
fn network_handler_without_shm_slice_penalized() {
    ...
    let f = make_fn("handler", vec![("slice", "ShmSlice"), ("extra", "String")], Some("VilResponse"), true);
    module.functions.push(f);

    let report = run_all_passes(&module);
    let has_zero_copy_issue = report.issues.iter().any(|i| i.contains("String") || i.contains("zero-copy"));
    assert!(
