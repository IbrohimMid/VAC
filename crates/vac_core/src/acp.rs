            ...
            "session/resume" => {
                ...
                if let Ok(sid) = Uuid::parse_str(sid_str) {
                    if let Some(session) = AcpSession::load(&sessions_dir, sid) {
                        ...
                        let _ = tx_writer
                            .send(serde_json::json!({ "id": id, "error": "Session not found" }));
                    }
                } else {
                    let _ = tx_writer
                        .send(serde_json::json!({ "id": id, "error": "Invalid session_id" }));
                }
            }
            ...
            "run_task" => {
                ...
                tokio::spawn(async move {
                    while let Some(event) = rx.recv().await {
                        let msg =
                            serde_json::json!({ "id": id_clone, "event": "update", "data": event });
                        if tx_writer_clone.send(msg).is_err() {
                            break;
                        }
                    }
                    let _ = tx_writer_clone
                        .send(serde_json::json!({ "id": id_clone, "event": "task_done" }));
                });
            }
...
pub fn runtime_update_to_acp_event(
    update: &crate::engine::RuntimeUpdate,
) -> Option<serde_json::Value> {
    use crate::engine::RuntimeUpdate::*;
    match update {
        Status(msg) => Some(serde_json::json!({ "event": "status", "data": { "message": msg } })),
        AssistantChunk(text) => {
            Some(serde_json::json!({ "event": "assistant_chunk", "data": { "text": text } }))
        }
        ToolCall {
            id,
            name,
