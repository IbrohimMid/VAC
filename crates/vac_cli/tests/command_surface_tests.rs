...
fn test_doctor_json_output() {
    ...
    assert!(parsed.get("checks").is_some(), "Missing 'checks' in doctor JSON");
    assert!(parsed.get("ready").is_some(), "Missing 'ready' in doctor JSON");
}
...
fn test_runtime_jobs_json_output() {
    ...
    let parsed: Value = serde_json::from_str(&stdout).expect("Failed to parse JSON output");
    assert!(parsed.get("jobs").is_some(), "Missing 'jobs' in runtime jobs JSON");
}

#[test]
fn test_autopilot_status_json_output() {
    let mut cmd = Command::cargo_bin("vac").unwrap();
    cmd.arg("autopilot").arg("status").arg("--format").arg("json");
    ...
    let parsed: Value = serde_json::from_str(&stdout).expect("Failed to parse JSON output");
    assert!(parsed.get("status").is_some(), "Missing 'status' in autopilot status JSON");
}
