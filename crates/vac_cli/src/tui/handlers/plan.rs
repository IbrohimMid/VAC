...
use crate::tui::app::{AppState, InputEvent};
...
pub fn handle_plan_review_key(state: &mut AppState, event: &InputEvent) -> bool {
    ...
    match event {
        ...
        InputEvent::InputChanged('a') => {
            write_plan_status(state, crate::tui::services::plan::PlanStatus::Approved);
            state.plan.review_open = false;
            state.add_assistant_message("Plan approved.".to_string());
        }
        InputEvent::InputChanged('r') => {
            write_plan_status(state, crate::tui::services::plan::PlanStatus::Drafting);
            state.plan.review_open = false;
...
pub fn write_plan_status(state: &mut AppState, new_status: crate::tui::services::plan::PlanStatus) {
