...
fn seed(paths: &VacPathsImpl) {
    let dir = paths.sessions_dir();
    fs::create_dir_all(&dir).unwrap();
    fs::write(dir.join("alpha.jsonl"), "operator: hi\nagent: hello\n").unwrap();
    fs::write(dir.join("beta.jsonl"), "operator: refactor\nagent: ok\n").unwrap();
}
...
fn delete_removes_transcript_file() {
    ...
    let remaining: Vec<String> =
        state.list(&paths).into_iter().map(|s| s.id).collect();
...
fn archive_moves_transcript_under_archive_dir() {
    ...
    assert!(paths.sessions_dir().join("archive").join("beta.jsonl").exists());
}
