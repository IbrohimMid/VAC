...
fn vil_dev_started_emits_activity_and_task_tray_entry() {
    ...
    handle_backend_event(
        &mut state,
        &tx,
        InputEvent::VilDevEvent(RunnerEvent::Started { pid: 42 }),
    );
    assert_eq!(state.vil_domain.vil_dev.pid, Some(42));
    assert!(
        state.vil_domain.vil_dev.job_id.is_some(),
        "task tray job must be created"
    );
    assert!(
...
fn vil_dev_exited_marks_task_tray_entry_done() {
    ...
    handle_backend_event(
        &mut state,
        &tx,
        InputEvent::VilDevEvent(RunnerEvent::Started { pid: 99 }),
    );
    let job_id = state
        .vil_domain
        .vil_dev
        .job_id
        .expect("job id must be set after Started");
    handle_backend_event(
        &mut state,
        &tx,
        InputEvent::VilDevEvent(RunnerEvent::Exited {
            code: Some(0),
            signal: None,
        }),
    );
    assert!(
        state.vil_domain.vil_dev.pid.is_none(),
        "pid must be cleared"
    );
    assert!(
        state.vil_domain.vil_dev.job_id.is_none(),
        "job id must be cleared after exit"
    );
    let job = state
        .execution
        .runtime
...
fn vil_dev_error_pushes_error_activity_and_task_tray_error() {
    ...
    handle_backend_event(
        &mut state,
        &tx,
        InputEvent::VilDevEvent(RunnerEvent::Started { pid: 7 }),
    );
