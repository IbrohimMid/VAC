...
fn contract_diff_scroll_down_increments_via_handler() {
    ...
    assert_eq!(ctx.state.workspace.changeset_ui.diff_scroll, 0);
    assert!(changeset::scroll_down(&mut ctx).is_ok());
    assert!(changeset::scroll_down(&mut ctx).is_ok());
    assert!(changeset::scroll_down(&mut ctx).is_ok());
    assert_eq!(ctx.state.workspace.changeset_ui.diff_scroll, 3);
}
...
fn contract_diff_navigation_pure_keyboard_no_events_emitted() {
    ...
    assert_eq!(ctx.state.workspace.changeset_ui.diff_scroll, 20);
    // Scroll back up.
    for _ in 0..10 {
        let _ = changeset::scroll_up(&mut ctx);
    }
    assert_eq!(ctx.state.workspace.changeset_ui.diff_scroll, 10);
}

#[test]
fn contract_diff_close_resets_selection_state() {
    let (mut state, tx) = test_ctx();
    state.workspace.changeset_ui.diff_scroll = 15;
    state.workspace.changeset_ui.selected_idx = 3;
    state.workspace.changeset_ui.selected_path = Some("src/foo.rs".into());

    let mut ctx = HandlerContext::new(&mut state, &tx);
    assert!(changeset::close(&mut ctx).is_ok());

    assert_eq!(ctx.state.workspace.changeset_ui.diff_scroll, 0);
    assert_eq!(ctx.state.workspace.changeset_ui.selected_idx, 0);
    assert!(ctx.state.workspace.changeset_ui.selected_path.is_none());
    assert!(ctx.state.workspace.changeset_ui.diff.is_none());
}
