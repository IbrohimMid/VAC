...
fn contract_attempt_quit_during_streaming_cancels_stream_not_app() {
    ...
    state.transcript.streaming.tokens = 42;
    ...
    assert_eq!(state.transcript.streaming.tokens, 0);
    assert!(state.transcript.streaming.start.is_none());
    assert!(
        !state.core.quit.cancel_requested,
...
fn contract_double_press_quit_requests_cancel() {
    ...
    handle_input_event(&mut state, &tx, InputEvent::AttemptQuit);
    assert_eq!(state.core.quit.press_count, 1, "first press records count");
    assert!(state.core.quit.first_press.is_some());
    assert!(!state.core.quit.cancel_requested);
    ...
    assert_eq!(state.core.quit.press_count, 0);
}
...
fn contract_diff_scroll_resets_when_new_file_selected() {
    ...
    state.workspace.changeset_ui.diff_scroll = 25;
    state.workspace.changeset_ui.selected_path = Some("src/a.rs".to_string());
