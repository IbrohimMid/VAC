            ...
            InputEvent::InputSubmitted => {
                if state.shortcuts_mode == crate::app::ShortcutsPopupMode::Sessions {
                    ...
                    if let Some(cmd) = cmds.get(state.shortcuts_scroll) {
                        if let crate::services::commands::CommandAction::InsertSlashCommand(s) = &cmd.action {
                                state.input.clear();
                ...
                crate::app::WorkspaceFocus::Workbench => match state.workbench_tab {
                    ...
                    crate::app::WorkbenchTab::Plan => match c {
                        'a' => {
                            plan_write_status(
                                state,
                                crate::services::plan::PlanStatus::Approved,
                            );
                        ...
                        'r' => {
                            plan_write_status(
                                state,
                                crate::services::plan::PlanStatus::Drafting,
                            );
            ...
            if !state.input.is_empty() {
                ...
                if msg.starts_with('/') {
                    ...
                    if !dispatch_builtin_command(state, output_tx, cmd_word, cmd_args) {
                        ...
                        state.pending_user_messages.push_back(crate::app::PendingUserMessage::new(
                            expanded.clone(),
                            None,
                            parts,
                            expanded,
...
fn plan_write_status(state: &mut AppState, new_status: crate::services::plan::PlanStatus) {
        ...
        match cmd.source {
            crate::app::CommandSource::BuiltIn => {
                if cmd.command == "/clear" {
                    ...
                    state
                        .messages
                        .extend(crate::services::helper_block::welcome_messages(
                            None, state,
                        ));
                        ...
                        crate::services::plan::read_plan_file(&project_root)
                    {
                        ...
                        if let Err(e) =
                            crate::services::plan::write_plan_file(&project_root, &tmpl)
                        {
