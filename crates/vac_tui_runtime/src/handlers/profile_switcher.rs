...
pub fn open(ctx: &mut HandlerContext) -> HandlerResult {
    ctx.state.layout.switchers.available_profiles = vec![
        "default".to_string(),
    ...
    ctx.state.layout.switchers.profile_search.clear();
    let filtered = ctx.state.profile_switcher_filtered();
    ctx.state.layout.switchers.profile_selected = filtered
        .iter()
        .position(|p| p == &ctx.state.layout.switchers.active_profile)
        .unwrap_or(0);
    Ok(())
}
...
pub fn select_next(ctx: &mut HandlerContext) -> HandlerResult {
    ...
    if !filtered.is_empty() {
        ctx.state.layout.switchers.profile_selected =
            (ctx.state.layout.switchers.profile_selected + 1).min(filtered.len().saturating_sub(1));
    }
    Ok(())
}
