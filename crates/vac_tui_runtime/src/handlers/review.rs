...
pub fn open(ctx: &mut HandlerContext) -> HandlerResult {
    ctx.state.layout.workbench_tab = crate::app::WorkbenchTab::Review;
...
pub fn revert_selected(ctx: &mut HandlerContext) -> HandlerResult {
    ...
    let Ok(session_id) = uuid::Uuid::parse_str(&ctx.state.session.session_id) else {
        ctx.state
...
pub fn revert_filtered(ctx: &mut HandlerContext) -> HandlerResult {
    ...
    for file in &files {
        match vac_tools::journal::restore_snapshot(&ctx.state.core.project_root, session_id, file) {
            Ok(()) => {
                success_count += 1;
...
pub fn revert_all(ctx: &mut HandlerContext) -> HandlerResult {
    let files = ctx.state.workspace.changeset_store.modified_files();
    if files.is_empty() {
        ctx.state
            .add_assistant_message("No files to revert.".to_string());
        return Ok(());
    ...
    ctx.state.workspace.modified_files = ctx.state.workspace.changeset_store.modified_files();
    ctx.state.workspace.review.diff = None;
    ctx.state.workspace.review.selected_idx = 0;
    ctx.state.workspace.review.selected_path = None;
    ctx.state
        .add_assistant_message(format!("Reverted {}/{} files.", success_count, files.len()));
    ctx.state.push_activity(
        ActivityKind::Review,
...
fn load_diff_for_selected(ctx: &mut HandlerContext) -> HandlerResult {
    if let (Some(path), Ok(session_id)) = (
        ctx.state.workspace.review.selected_path.clone(),
        uuid::Uuid::parse_str(&ctx.state.session.session_id),
    ) {
        match review::load_diff(&ctx.state.core.project_root, session_id, &path) {
            Ok(diff) => {
                ctx.state.workspace.review.diff = Some(crate::app::ReviewDiffState {
                    path: diff.path,
...
pub fn filter_push(ctx: &mut HandlerContext, c: char) -> HandlerResult {
    ctx.state.workspace.review.filter.push(c);
    ctx.state.review_normalize_selection();
    if ctx.state.workspace.review.diff.is_some() {
        load_diff_for_selected(ctx)?;
    }
    Ok(())
}
...
pub fn filter_pop(ctx: &mut HandlerContext) -> HandlerResult {
    ctx.state.workspace.review.filter.pop();
    ctx.state.review_normalize_selection();
    if ctx.state.workspace.review.diff.is_some() {
        load_diff_for_selected(ctx)?;
    }
...
pub fn scroll_up(ctx: &mut HandlerContext, step: usize) -> HandlerResult {
    if let Some(diff) = &mut ctx.state.workspace.review.diff {
...
pub fn scroll_down(ctx: &mut HandlerContext, step: usize) -> HandlerResult {
    if let Some(diff) = &mut ctx.state.workspace.review.diff {
        ...
        if let (Some(old), Some(new)) = (diff.old_content.as_deref(), diff.new_content.as_deref()) {
            let total = crate::services::file_diff::render_diff(
