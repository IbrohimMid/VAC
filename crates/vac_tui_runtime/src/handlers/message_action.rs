...
use crate::services::message_action_popup::MessageAction;
...
pub fn handle_event(ctx: &mut HandlerContext, event: InputEvent) -> HandlerResult {
    match event {
        ...
        InputEvent::Up => {
                ...
                > 0
            {
                ...
                ctx.state
                    .operator_config
                    .operator
                    .message_action_popup_selected = num_actions.saturating_sub(1);
            }
        }
        InputEvent::Down => {
            let num_actions = MessageAction::all().len();
            if num_actions > 0 {
                ctx.state
                    .operator_config
                    .operator
                    .message_action_popup_selected = (ctx
                    .state
                    .operator_config
                    .operator
                    .message_action_popup_selected
        ...
        InputEvent::InputSubmitted => {
            ...
            if let Some(action) = actions.get(
                ctx.state
                    .operator_config
                    .operator
                    .message_action_popup_selected,
