...
pub fn handle_ask_user(state: &mut AppState, output_tx: &Sender<OutputEvent>, event: InputEvent) {
    {
        ...
        let has_options = !state.layout.ask_user.options.is_empty();
        ...
        match event {
            InputEvent::HandleEsc => {
                ...
                if let Some(tc_id) = state.layout.ask_user.tool_call_id.take() {
                ...
                state.layout.ask_user.question = None;
                state.layout.ask_user.options.clear();
                state.layout.ask_user.input.clear();
                state.layout.ask_user.multi_selected.clear();
                state.layout.ask_user.metadata.clear();
            ...
            InputEvent::Tab => {
                if has_options {
                    state.layout.ask_user.search_active = !state.layout.ask_user.search_active;
                }
            }
            InputEvent::Up | InputEvent::ScrollUp => {
                if !filtered.is_empty() && selected_pos > 0 {
                    let new_pos = selected_pos - 1;
                    state.layout.ask_user.selected = filtered[new_pos];
                    state.layout.ask_user.scroll = state.layout.ask_user.scroll.min(new_pos);
                }
            }
            InputEvent::Down | InputEvent::ScrollDown => {
                if !filtered.is_empty() && selected_pos + 1 < filtered.len() {
                    ...
                    state.layout.ask_user.scroll = state.layout.ask_user.scroll.min(max_scroll);
                    if new_pos >= state.layout.ask_user.scroll.saturating_add(max_visible) {
                        state.layout.ask_user.scroll = new_pos + 1 - max_visible;
                    }
                }
            }
            InputEvent::InputChanged(c) => {
                if state.layout.ask_user.search_active {
                    state.layout.ask_user.filter.push(c);
                    state.layout.ask_user.scroll = 0;
                    let filtered = crate::services::ask_user::filtered_option_indices(
                        &state.layout.ask_user.filter,
                        &state.layout.ask_user.options,
                    ...
                    if let Some(&first) = filtered.first() {
                        state.layout.ask_user.selected = first;
                    }
                    ...
                    && !filtered.is_empty()
                {
                    ...
                    if state.layout.ask_user.input.is_empty() && c.is_ascii_digit() && c != '0' {
                        let idx = (c as u8 - b'1') as usize;
                        if idx < filtered.len() {
                            state.layout.ask_user.selected = filtered[idx];
                            return;
                        }
                    }
                    // Honor `allow_free_text`: when the caller disabled it,
                    // typed characters that aren't number shortcuts are dropped.
                    if state.layout.ask_user.allow_free_text {
                        state.layout.ask_user.input.push(c);
                    }
                }
            }
            InputEvent::InputBackspace => {
                if state.layout.ask_user.search_active {
                    state.layout.ask_user.filter.pop();
                    state.layout.ask_user.scroll = 0;
                    let filtered = crate::services::ask_user::filtered_option_indices(
                        &state.layout.ask_user.filter,
                        &state.layout.ask_user.options,
                ...
                } else if state.layout.ask_user.allow_free_text {
                    state.layout.ask_user.input.pop();
                }
            }
            ...
            InputEvent::InputCursorStart => {
                if kind == crate::services::ask_user::AskUserQuestionKind::MultiSelect {
                    let indices = if state.layout.ask_user.filter.trim().is_empty() {
                        (0..state.layout.ask_user.options.len()).collect::<Vec<_>>()
                    } else {
                        filtered.clone()
                    };
                    state.layout.ask_user.multi_selected = indices.into_iter().collect();
                }
            }
            InputEvent::InputSubmitted => {
                if let Some(tc_id) = state.layout.ask_user.tool_call_id.take() {
                    ...
                    let payload = crate::services::ask_user::build_answer(
                        state,
                        &state.layout.ask_user.multi_selected,
                    );
                    let answer =
