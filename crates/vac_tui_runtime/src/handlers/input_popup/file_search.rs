...
pub(super) fn handle_file_search(
    ...
    match event {
        ...
        InputEvent::InputChanged(c) => {
            let mut q = ctx.state.workspace.file_index.search_query.clone();
            q.push(c);
...
pub(super) fn handle_file_picker(
    ...
    match event {
        ...
        InputEvent::Down | InputEvent::ScrollDown => {
            let len = state.workspace.file_picker.results.len();
            if len > 0 {
                state.workspace.file_picker.selected =
                    (state.workspace.file_picker.selected + 1).min(len.saturating_sub(1));
                update_file_picker_preview(state);
            }
        }
        // Space toggles multi-selection
        InputEvent::InputChanged(' ') => {
            let idx = state.workspace.file_picker.selected;
            if state.workspace.file_picker.multi_selected.contains(&idx) {
                ...
                .unwrap_or(false)
            {
                state.workspace.file_picker.multi_selected.insert(idx);
            }
        }
        ...
        InputEvent::Tab => {
            ...
            {
                if path.is_dir() {
                    state.workspace.file_picker.cwd = path;
        ...
        InputEvent::InputBackspace => {
            if state.workspace.file_picker.query.is_empty() {
                ...
                state.workspace.file_picker.query.pop();
                state.workspace.file_picker.selected = 0;
                refresh_file_picker_results(state);
            }
        }
        InputEvent::InputChanged(ch) => {
            state.workspace.file_picker.query.push(ch);
            state.workspace.file_picker.selected = 0;
            refresh_file_picker_results(state);
        }
        InputEvent::InputSubmitted => {
...
fn refresh_file_picker_results(state: &mut AppState) {
    let query = state.workspace.file_picker.query.to_lowercase();
    let type_filter = state.workspace.file_picker.type_filter.clone();
    let cwd = state.workspace.file_picker.cwd.clone();
    ...
    state.workspace.file_picker.results = results;
    update_file_picker_preview(state);
}
...
fn update_file_picker_preview(state: &mut AppState) {
    ...
    state.workspace.file_picker.preview = preview;
}
