...
pub(super) fn handle_file_picker(
    ...
    match event {
        ...
        InputEvent::Up | InputEvent::ScrollUp => {
            state.file_picker.selected = state.file_picker.selected.saturating_sub(1);
            update_file_picker_preview(state);
        }
        InputEvent::Down | InputEvent::ScrollDown => {
            let len = state.file_picker.results.len();
            if len > 0 {
                state.file_picker.selected =
                    (state.file_picker.selected + 1).min(len.saturating_sub(1));
                update_file_picker_preview(state);
            }
        }
        // Space toggles multi-selection
        InputEvent::InputChanged(' ') => {
            let idx = state.file_picker.selected;
            if state.file_picker.multi_selected.contains(&idx) {
                state.file_picker.multi_selected.remove(&idx);
            } else if state
                .file_picker.results
                .get(idx)
                .map(|p| p.is_file())
                .unwrap_or(false)
            {
                state.file_picker.multi_selected.insert(idx);
            }
        }
        ...
        InputEvent::Tab => {
            if let Some(path) = state
                .file_picker.results
                .get(state.file_picker.selected)
                .cloned()
            {
                if path.is_dir() {
                    state.file_picker.cwd = path;
        ...
        InputEvent::InputBackspace => {
            if state.file_picker.query.is_empty() {
                if let Some(parent) = state.file_picker.cwd.parent().map(|p| p.to_path_buf()) {
                    state.file_picker.cwd = parent;
                    state.file_picker.selected = 0;
                    refresh_file_picker_results(state);
                }
            } else {
                state.file_picker.query.pop();
                state.file_picker.selected = 0;
                refresh_file_picker_results(state);
            }
        }
        InputEvent::InputChanged(ch) => {
            state.file_picker.query.push(ch);
            state.file_picker.selected = 0;
            refresh_file_picker_results(state);
        }
        InputEvent::InputSubmitted => {
            let selected: Vec<std::path::PathBuf> = if state.file_picker.multi_selected.is_empty() {
                state
                    .file_picker.results
                    .get(state.file_picker.selected)
                    .filter(|p| p.is_file())
                ...
                let mut sel: Vec<_> = state.file_picker.multi_selected.iter().copied().collect();
                sel.sort();
                sel.into_iter()
                    .filter_map(|i| state.file_picker.results.get(i))
                    .filter(|p| p.is_file())
                    .cloned()
                    .collect()
            };
...
fn refresh_file_picker_results(state: &mut AppState) {
    let query = state.file_picker.query.to_lowercase();
    let type_filter = state.file_picker.type_filter.clone();
    let cwd = state.file_picker.cwd.clone();
    ...
    state.file_picker.results = results;
    update_file_picker_preview(state);
}
...
fn update_file_picker_preview(state: &mut AppState) {
    ...
    state.file_picker.preview = preview;
}
