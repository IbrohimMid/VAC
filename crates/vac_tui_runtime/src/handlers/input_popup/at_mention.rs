    ...
    match event {
        InputEvent::Up => {
            state.composer.at_mention.selected_idx =
                state.composer.at_mention.selected_idx.saturating_sub(1);
        }
        InputEvent::Down => {
            if !state.composer.at_mention.results.is_empty() {
                state.composer.at_mention.selected_idx = (state.composer.at_mention.selected_idx
                    + 1)
        ...
        InputEvent::InputSubmitted => {
            if let Some(path) = state
                ...
                .at_mention
                .results
                .get(state.composer.at_mention.selected_idx)
                .cloned()
            {
                ...
                let (namespace, label) =
                    parse_at_namespace(&state.composer.at_mention.query, &path);
                ...
                state
                    .composer
                    .context_chips
                    .push(crate::app::types::ContextChip {
                        label,
                        content,
                        namespace,
                    });
