...
| D7B | `vac_shell_host_vac_command_adapter` | Second ADR-sanctioned host-side exception (`vac_session_engine` dep). Implements `vac_shell_host_commands::ShellCommandExecutor` by submitting mapped custom palette slashes through `vac_session_engine::submit_one` with the `EchoAdapter` LLM stub. Built-in slashes still go through `ShellApp::apply_event`; unmapped customs return `ShellCommandError::Unsupported`. Sync trait kept; async resolved internally via `block_in_place` / current-thread fallback. Dogfood example wires the adapter as a dev-dep path; the entrypoint library does not depend on it. |
| D7C | Same crate, real provider routing. Adds `AdapterLlm::{Echo, Custom}`, `AdapterConfig::with_llm` / `with_vil_llm_router`, and a `VilLlmRouterAdapter` bridge wrapping `vil_llm::LlmRouter` as `vac_session_engine::LlmAdapter`. Default stays `Echo`; opt-in `Custom` lets hosts inject any adapter (cassette, custom routing, or vil_llm). Provider-level errors surface as `ShellCommandError::Failed`. Provider identity preserved through fallback via `LlmRouter::complete_with_provider`. |
| D7D | Same crate, tool-use round-tripping. `VilLlmRouterAdapter` now translates `vil_llm::ToolCall { id, name, arguments }` into `vac_session_engine::ToolCallRequest { id, name, arguments, reason: None, estimated_tokens: 0 }`. Engine's `UnsupportedDispatcher` keeps the no-dispatcher path safe — tool calls produce error envelopes without aborting the submit. Real tool dispatch remains opt-in via host-attached `ToolDispatcher` + `CompositeGate` on `CompactConfig`. |
| D8 | New host-side bridge crate `vac_shell_host_vac_tool_dispatcher`. Third ADR-sanctioned exception (depends on `vac_session_engine` + `vac_tools` + `vac_tool_core`). Ships `VacToolDispatcher` impl `ToolDispatcher` over `ToolRegistry`. Adapter gains `with_tool_dispatcher(dispatcher, gate)` (infallible) + `try_with_tool_dispatcher(dispatcher, Option<gate>)` (pre-flight rejects dispatcher without gate). Default stays `UnsupportedDispatcher`; opt-in dogfood example `dogfood_tool_dispatch` registers a real read-only tool. D8E `read_tool_use_rows` reads paired `tool_call`/`tool_result` views from any transcript, tolerant of pre-D7E sessions. |
| D9 | New host-side projection crate `vac_shell_host_transcript_projection`. Fourth ADR-sanctioned exception (read-only `vac_session_engine` consumer). `project_tool_use_activity` / `summarize_tool_use` / `session_tool_use_summary` turn D7E/D8 `tool_call`/`tool_result` rows into `ShellActivityEntry` + a count summary for the session-browser tile. Severity map: `Ok→Ok`, `Warning→Warn`, `Error→Error`, `Cancelled→Warn`, missing→`Pending`/Warn. Operator redaction: only tool name, status, envelope summary, duration, transcript path land in the projected text — raw `payload` and `arguments` never leave the JSONL on disk. Tolerant of missing files, pre-D7E transcripts, and partial transcripts. |
